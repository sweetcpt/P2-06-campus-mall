  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/participant' && request.method === 'POST') {
      ...
      await env.DB.prepare(`
        ...
        (study_key,participant_id,variant,status,started_at,cart_opened_at,baseline_value,final_value,addon_value,initial_quantity,final_quantity,continued_add,checkout_clicked,free_shipping,completed_at,cart_json,updated_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
      ...
      `).bind(
        studyKey, text(r.participantId,64), r.variant, r.status, text(r.startedAt,40), text(r.cartOpenedAt,40),
